...
class Approach(ABC):
    ...
    def get_citation(self, sourcepage: str, use_image_citation: bool, metadata: Optional[dict[str, Any]] = None) -> str:
        """
        Generate a citation for a source page.
        
        Args:
            sourcepage: The source page/file path to generate a citation for
            use_image_citation: Whether this is for an image citation
            metadata: Optional metadata about the document (unused in basic implementation)
            
        Returns:
            The citation URL/reference
        """
        if use_image_citation:
            # For image citations, return the source page as-is
            return sourcepage
        ...
        if ext.lower() == ".png":
            ...
            if page_idx != -1:
                try:
                    ...
                    # Return PDF path with page fragment
                    return f"{pdf_path}#page={page_number}"
                except (ValueError, IndexError):
                    pass
            
            # Fallback to original sourcepage if page extraction fails
            return sourcepage
        
        # For all other file types, return the sourcepage as-is
        return sourcepage
